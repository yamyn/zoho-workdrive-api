...
class WorkdriveController {
    ...
    ps = {
        info: async ({ wsId, accessToken, domain }) => {
            accessToken = accessToken ? accessToken : this.accessToken;
            domain = domain ? domain : this.domain;

            return this.wdService.privatespaceService.getPrivateFolder(wsId, accessToken, domain);
        },
        files: async ({ psId, accessToken, domain }) => {
            accessToken = accessToken ? accessToken : this.accessToken;
            domain = domain ? domain : this.domain;
    ...
    files = {
        create: async ({ parentId, name, zFileType, accessToken, domain }) => {
            accessToken = accessToken ? accessToken : this.accessToken;
            domain = domain ? domain : this.domain;

            return this.wdService.filesService.createFile(parentId, name, zFileType, accessToken, domain);
        },
        upload: async ({ parentId, name, overrideNameExist, readableStream, accessToken, domain }) => {
            accessToken = accessToken ? accessToken : this.accessToken;
            domain = domain ? domain : this.domain;
